...
public:
  ...
  void computeNBVFrontiersToVisit();
  list<Frontier> nbv_frontiers;

  void getFrontiers(vector<vector<Vector3d>>& clusters);
  void getnbvFrontiers(vector<Vector3d>& points_,vector<double>& yaws_);
  ...
  */
  void getTopViewpointsInfo(const Vector3d& cur_pos, vector<Vector3d>& points, vector<double>& yaws,
                            vector<Vector3d>& averages,vector<int>& ids_);
  ...
  void findViewpoints(const Vector3d& sample, const Vector3d& ftr_avg, int& slince_num);
