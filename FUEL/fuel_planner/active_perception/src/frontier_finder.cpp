...
//自己加的
  ...
  //开始随机撒点
  //首先求解局部地图总共存在多少栅格点
  int num_local=(max_id(0)-min_id(0))*(max_id(1)-min_id(1))*(max_id(2)-min_id(2));//局部栅格数
  //之后根据地图大小设置撒点数，首先撒一半点，即1/2
  size_t M = num_local/200;//随机撒点数，应该根据局部地图大小决定
  //size_t M=300;
  ROS_ERROR("M value: %d",M);
  //撒点根据栅格坐标撒点，即不用实际坐标，这样可以节省一次从实际点坐标转换值栅格坐标的运算
  int x,y,z;//随机撒点的栅格坐标
  //这只随机种子值
  srand(time(0));//利用时间作为随机种子，这样可以每次随机数都不同
  for (size_t i = 0; i < M; i++)
  {
    int x = min_id(0) + rand() % (max_id(0)-min_id(0));
    int y = min_id(1) + rand() % (max_id(1)-min_id(1));
    int z = min_id(2) + rand() % (max_id(2)-min_id(2));
